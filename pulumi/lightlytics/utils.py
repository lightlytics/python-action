...
def get_resource_name_by_convention(name, suffix=None):
    ...
    # prod-lightlytics-<name>
    resource_name = f"{stack_name}-{project_name}-{name}"
...
def get_subnet_name_by_convention(subnet_type, suffix, vpc, zone_name):
    partial_name, _ = get_resource_name_by_convention(subnet_type, suffix)
    name = f'{partial_name}-{zone_name}'
    name_tag = pulumi.Output.concat(vpc.tags['Name'], "-", name)
    tags = _eks_public_subnet_tags(name_tag)
    if subnet_type == PRIVATE_SUBNET_TYPE_NAME:
        tags = _eks_private_subnet_tags(name_tag)
    return name, tags
...
def _eks_private_subnet_tags(name_tag):
    return {
        ...
        'kubernetes.io/cluster/prod-lightlytics-eks_cluster': "shared"
    }
