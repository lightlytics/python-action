...
def get_lightlytics_vpc_and_network():
    vpc = ec2.get_vpc(tags={'Name': 'prod-lightlytics-vpc-us-east-1'})
    private_subnets = [
        ec2.get_subnet(tags={'Name': 'prod-lightlytics-vpc-us-east-1-prod-lightlytics-private_subnet_1-us-east-1a'}),
        ec2.get_subnet(tags={'Name': 'prod-lightlytics-vpc-us-east-1-prod-lightlytics-private_subnet_2-us-east-1b'}),
        ec2.get_subnet(tags={'Name': 'prod-lightlytics-vpc-us-east-1-prod-lightlytics-private_subnet_3-us-east-1c'}),
    ]
    public_subnets = [
        ec2.get_subnet(tags={'Name': 'prod-lightlytics-vpc-us-east-1-prod-lightlytics-public_subnet_1-us-east-1a'}),
        ec2.get_subnet(tags={'Name': 'prod-lightlytics-vpc-us-east-1-prod-lightlytics-public_subnet_1-us-east-1a'}),
        ec2.get_subnet(tags={'Name': 'prod-lightlytics-vpc-us-east-1-prod-lightlytics-public_subnet_3-us-east-1c'}),
    ]
    return {
        VPC_TYPE_NAME: vpc,
...
def create_vpc_and_subnets_per_external_regions():
    ...
    for region in regions:
        ...
        vpc = _create_vpc_external(region, provider)
...
def _create_vpc(region_name, provider=None):
...
def _create_vpc_external(region_name, provider=None):
    name, name_tag = get_resource_name_by_convention(f'vpc-{region_name}')
    vpc = ec2.Vpc(
        name,
        cidr_block=conf.require(f'VpcBlock-{region_name}'),
...
def _create_subnets_for_zones(subnet_type, subnet_networks, vpc, zone_names):
    ...
    for i, zone_name in enumerate(zone_names):
        name, tags = get_subnet_name_by_convention(subnet_type, i, zone_name)
...
def _create_subnets_for_zones_on_external_network(subnet_type, subnet_networks, vpc, zone_names, provider, region_name):
    ...
    for i, zone_name in enumerate(zone_names):
        ...
        subnets.append(subnet)
        pulumi.export(f'{subnet_type}-{i}-{region_name}', subnet.id)
    return subnets
