...
def get_iam_roles():
    role_list = [
        {'name': 'ms_customers_role', 'id': 'prod-lightlytics-ms_customers_role'},
        {'name': 'ms_account_role', 'id': 'prod-lightlytics-ms_account_role'},
        {'name': 'ms_changes_role', 'id': 'prod-lightlytics-ms_changes_role'},
        {'name': 'cronjob_mongo_backup_role', 'id': 'prod-lightlytics-cronjob_mongo_backup_role'}
    ]
    for role in role_list:
        iam_role = iam.Role.get(
            role.get('name'),
            id=role.get('id')
        )
        pulumi.export(role.get('name'), iam_role.arn)


def get_eks_cluster():
    name, _ = get_resource_name_by_convention('eks_cluster')
    eks_cluster = eks.Cluster.get(
        name,
        id="prod-lightlytics-eks_cluster"
    )
    get_iam_roles()
...
def create_eks_cluster(vpc, public_subnets, private_subnets, keypair):
    ...
    attach_ms_accounts_service_role_policies(created_roles)
    attach_customers_service_role_policies(created_roles)
    attach_ms_changes_role_policies(created_roles)
    attach_alb_ingress_controller_role_policies(created_roles)
    attach_external_secrets_service_role_policies(created_roles)
    attach_external_dns_role_policies(created_roles)
    attach_cron_mongo_backup_role_policy(created_roles)
...
def create_eks_sts_assume_role_policy():
    ...
    with open('polices/eks_assume_role_policy.json') as f:
        ...
        eks_cluster_iam_role = iam.Role(
            name,
            assume_role_policy=eks_assume_role_policy_document,
            tags=tags
        )
...
def create_eks_cluster_nodes_iam_role():
    with open('polices/assume_role_policy.json') as f:
        ...
        eks_cluster_node_group_iam_role = iam.Role(
            name,
            tags=tags,
            assume_role_policy=eks_node_group_assume_role_policy
        )
...
def attach_customers_service_role_policies(service_accounts_roles):
    ...
    iam.RolePolicyAttachment(
        name,
        role=role.name,
        policy_arn="arn:aws:iam::624907860825:policy/prod-lightlytics-ses_policy-9b0a45d"
    )
    name, _ = get_resource_name_by_convention('sqs_to_ms_customers_attach_policy')
    iam.RolePolicyAttachment(
        name,
        role=role.name,
        policy_arn="arn:aws:iam::624907860825:policy/prod-lightlytics-sqs_policy-b7897e5"
    )
    pulumi.export("ms_customers_role", role.arn)


def attach_ms_accounts_service_role_policies(service_accounts_roles):
    ...
    iam.RolePolicyAttachment(
        name,
        role=role.name,
        policy_arn="arn:aws:iam::624907860825:policy/prod-lightlytics-s3_write_access_iam_policy-cft-c0f17e1"
    )
    ...
    iam.RolePolicyAttachment(
        name,
        role=role.name,
        policy_arn="arn:aws:iam::624907860825:policy/prod-lightlytics-describe_regions_policy-1ce27aa"
    )
    ...
    iam.RolePolicyAttachment(
        name,
        role=role.name,
        policy_arn="arn:aws:iam::624907860825:policy/prod-lightlytics-sts_policy-00ad33a"
    )
    pulumi.export("ms_account_role", role.arn)


def attach_ms_changes_role_policies(service_accounts_roles):
    ...
    iam.RolePolicyAttachment(
        name,
        role=role.name,
        policy_arn="arn:aws:iam::624907860825:policy/prod-lightlytics-sqs_messages_policy-ea2b5e6"
    )

    pulumi.export("ms_changes_role", role.arn)


def attach_alb_ingress_controller_role_policies(service_accounts_roles):
    ...
    iam.RolePolicyAttachment(
        name,
        role=role,
        policy_arn="arn:aws:iam::624907860825:policy/prod-lightlytics-alb_ingress_controller_policy-232f574"
    )
    pulumi.export("alb_ingress_controller_iam_role", role.arn)


def attach_external_secrets_service_role_policies(service_accounts_roles):
    ...
    iam.RolePolicyAttachment(
        name,
        role=role,
        policy_arn="arn:aws:iam::624907860825:policy/prod-lightlytics-secret_manager_policy-8aa8294"
    )
    ...
    iam.RolePolicyAttachment(
        name,
        role=role,
        policy_arn="arn:aws:iam::624907860825:policy/prod-lightlytics-ssm_policy-dd359c2"
    )
    pulumi.export("external_secrets_role", role.arn)


def attach_external_dns_role_policies(service_accounts_roles):
    ...
    iam.RolePolicyAttachment(
        name,
        role=role,
        policy_arn="arn:aws:iam::624907860825:policy/prod-lightlytics-external_dns_policy-c2cf4d2"
    )
    pulumi.export("external_dns_role", role.arn)


def attach_cron_mongo_backup_role_policy(service_accounts_roles):
    role = service_accounts_roles['cronjob_mongo_backup_role']
    name, _ = get_resource_name_by_convention("s3_to_cronjob_mongo_backups")
    iam.RolePolicyAttachment(
        name,
        role=role.name,
        policy_arn="arn:aws:iam::624907860825:policy/prod-lightlytics-s3_write_access_iam_policy-mongo-3600ab7"
    )
